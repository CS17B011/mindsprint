            ...
            smtp({
              ...
              auth: {
                xoauth2: xoauth2.createXOAuth2Generator({
                  ...
                  clientId:
                    "141530558274-mcm7tdqrfl0rmu1cecghbsrr3ar1arcb.apps.googleusercontent.com",
              ...
              if (!response) {
                await transporter.sendMail(mailOptions, async function(
                  ...
                  try {
                    ...
                    res.status(200).json({
                      success:
                        "Email has been sent successfully for email Verification"
                    });
                  } catch (err) {
...
exports.googleOAuth = function(req, res, next) {
  ...
  res.status(200).json({
    token: TokenForUser(req.user),
    success: "You are successfully logged in using google account",
    emailId: req.user.emailId
  });
};
...
exports.facebookOAuth = function(req, res, next) {
  ...
  res.status(200).json({
    token: TokenForUser(req.user),
    success: "You are successfully logged in using facebook account",
    emailId: req.user.emailId
  });
};
...
exports.getVerificationResult = async function(req, res) {
  ...
  try {
    ...
    if (user.active === true) {
      return res
        .status(200)
        .json({ success: "Email has already been verified" });
    }
    user.active = true;
    user.secretToken = "";
    await user.save();
